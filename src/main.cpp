#include "QY_FaceOperator.hpp"
#include "QY_CameraOperator.hpp"
...
{
    bool loadResult = QY_FaceOperator::loadCascadeClassifier("./resource/haarcascade_frontalface_alt.xml");
    if(false == loadResult){
        std::cerr << "load cascadfliePathe classier failed\n";
        exit(EXIT_FAILURE);
    }

    QY_CameraOperator::Handler handle = [](cv::Mat mat){
        cv::imshow("use camera", mat);
        cv::waitKey(1);
        std::thread task(&QY_FaceOperator::faceDetect, mat, 1.0, true, detectResultHandle);
        task.detach();
    };

    QY_CameraOperator::handleFrameFromUSBCamera(handle);
